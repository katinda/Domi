...
export default function Women({ LinkBox }) {
    ...
    const handleFormSubmit = (filteredData) => {
        setFilteredCandidates(filteredData.filter(candidate => candidate.gender.toLowerCase() === 'male'));
        ...
        <div>
            ...
            <section>
                ...
                <ConditionalDisplay active={contentToDisplay !== '' && !formSubmitted}>
                    <Pictures
                        candidates={candidates.filter(candidate => candidate.firstName.charAt(0).toUpperCase() === contentToDisplay && candidate.gender.toLowerCase() === 'male')} />
                </ConditionalDisplay>
            </section>
        </div>
    )
